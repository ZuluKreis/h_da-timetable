...
export default function App() {
    ...
    const [activeWeekFilter, setActiveWeekFilter] =
        useState<WeekFilter>("Alle");
    const [activePriorityFilter, setActivePriorityFilter] =
        useState<PriorityFilter>("Alle");
    ...
    useEffect(() => {
        ...
        async function loadEvents() {
            try {
                ...
                if (!ignore) {
                    setSaveError(
                        "JSON-Speicher nicht erreichbar. Die Daten werden nicht in eine Datei geschrieben.",
                    );
                }
            } finally {
                if (!ignore) {
    ...
    const handleSave = (event: FormEvent<HTMLFormElement>) => {
        ...
        const nextEvents = editingEvent
            ? events.map((entry) =>
                  entry.id === editingEvent
                      ? { ...formData, id: editingEvent }
                      : entry,
              )
    ...
    const matchesPriorityFilter = (entry: ScheduleEvent) => {
        ...
        return (
            entry.priority ===
            Number.parseInt(activePriorityFilter.slice(1), 10)
        );
    };
                ...
                <div className="mb-8 flex flex-col justify-between gap-4 md:flex-row md:items-end">
                    <div>
                        ...
                        <p className="mt-1 text-gray-500">
                            Plane deine Vorlesungen und Praktika effizient.
                        </p>
                        <p className="mt-2 text-sm text-gray-500">
                            {isLoading
                                ? "Lade Stundenplan aus data/events.json ..."
                                : "Speicherort: data/events.json"}
                        </p>
                        {saveError && (
                ...
                <div className="overflow-x-auto rounded-xl border border-gray-200 bg-white shadow-sm">
                    <div className="min-w-225">
                        <div className="grid grid-cols-[100px_repeat(5,minmax(0,1fr))] border-b border-gray-200 bg-gray-100">
                            <div className="p-3 text-center text-sm font-semibold uppercase tracking-wider text-gray-600">
                                Zeit
                            </div>
                            {DAYS.map((day) => (
                                <div
                                    key={day}
                                    className="min-w-0 border-l border-gray-200 p-3 text-center font-semibold text-gray-800"
                                >
                                    {day}
                                </div>
                            ))}
                        ...
                        {TIMES.map((timeLabel, timeIndex) => (
                            <Fragment key={timeLabel}>
                                <div
                                    className={`grid grid-cols-[100px_repeat(5,minmax(0,1fr))] ${timeIndex === 2 ? "border-b-0" : "border-b border-gray-100"} last:border-0`}
                                >
                                    ...
                                    {DAYS.map((_, dayIndex) => {
                                        ...
                                        return (
                                            <div
                                                ...
                                                className="group relative flex min-h-30 min-w-0 cursor-pointer flex-col border-l border-gray-200 p-1.5 transition-colors hover:bg-gray-50"
                                                onClick={() =>
                                                    openModal(
                                                        dayIndex,
                                                        timeIndex,
                                                    ...
                                                    {cellEvents.map((entry) => (
                                                        ...
                                                        >
                                                            <div className="mb-1 flex items-start justify-between">
                                                                <span className="truncate pr-6 font-bold leading-tight">
                                                                    {
                                                                        entry.title
                                                                    }
                                                                </span>
                                                                {entry.type !==
                                                                    "Vorlesung" && (
                                                                    <span
                                                                        className={`absolute right-2 top-2 rounded px-1.5 py-0.5 text-[10px] font-bold shadow-sm ${getPrioColors(entry.priority)}`}
                                                                    >
                                                                        P
                                                                        {
                                                                            entry.priority
                                ...
                                {timeIndex === 2 && (
                                    <div className="grid grid-cols-[100px_repeat(5,minmax(0,1fr))] border-y border-dashed border-amber-200 bg-amber-50/60">
                                        <div className="px-3 py-2 text-center text-xs font-semibold uppercase tracking-wide text-amber-700">
                                            13:30 - 14:15
                                        </div>
                                        <div className="col-span-5 flex items-center justify-center border-l border-amber-200 px-3 py-2">
                                            <span className="rounded-full border border-amber-300 bg-white/80 px-3 py-1 text-xs font-semibold text-amber-700 shadow-sm">
                                                Mittagspause
                                            </span>
                                        </div>
                                    </div>
                                        ...
                                        {PRIORITIES.map((priority) => (
                                            ...
                                            >
                                                ...
                                                <span className="flex items-center gap-2 text-sm">
                                                    Prio {priority}
                                                    <span
                                                        className={`h-2.5 w-2.5 rounded-full ${priority === 1 ? "bg-red-500" : priority === 2 ? "bg-orange-500" : "bg-yellow-500"}`}
                                                    ></span>
                                                </span>
                                            </label>
                                        ))}
                ...
                <div className="fixed inset-0 z-60 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm">
                    <div className="w-full max-w-sm overflow-hidden rounded-xl bg-white p-6 text-center shadow-xl duration-200 animate-in fade-in zoom-in-95">
                        ...
                        <p className="mb-6 text-gray-500">
                            Mochtest du diesen Eintrag wirklich aus deinem
